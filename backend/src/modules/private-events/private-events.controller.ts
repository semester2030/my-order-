...
export class PrivateEventsController {
  ...
  async create(@Request() req: { user: User }, @Body() dto: CreatePrivateEventRequestDto) {
    return this.privateEventsService.createPrivateEventRequest(req.user.id, dto);
  }
  ...
  async getMine(@Request() req: { user: User }) {
    return this.privateEventsService.findPrivateEventRequestsByUser(req.user.id);
  }
}
